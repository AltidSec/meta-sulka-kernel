...
`recipes-kernel/linux/linux-yocto_6.6.bbappend` inherits that class for the kernel version Sulka targets.
...
That bbappend wildcards the patch release of the `procps` version it targets, so it should apply to `scarthgap` recipes regardless of the exact used Yocto version.
...
| Target kernel | `linux-yocto` 6.6 in the default Sulka build |
